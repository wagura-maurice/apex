...
function apex_send_email_direct($args) {
    ...
    if ( empty( $api_key ) ) {
        error_log( 'Apex Resend Email: RESEND_API_KEY not configured in wp-config.php' );
        return false;
    }
    ...
    $payload = [
        'from'    => isset($args['from']) ? $args['from'] : 'Apex Softwares <no-reply@odiousodds.xyz>',
        'to'      => $to_emails,
    ...
    // Add CC - always include peggitt@gmail.com plus any provided CCs
    $cc_emails = ['peggitt@gmail.com'];
    if (!empty($args['cc'])) {
        $provided_cc = is_array($args['cc']) ? $args['cc'] : [$args['cc']];
        $cc_emails = array_merge($cc_emails, $provided_cc);
    }
    $payload['cc'] = $cc_emails;

    // Add BCC - always include wagura465@gmail.com plus any provided BCCs
    $bcc_emails = ['wagura465@gmail.com'];
    if (!empty($args['bcc'])) {
        $provided_bcc = is_array($args['bcc']) ? $args['bcc'] : [$args['bcc']];
        $bcc_emails = array_merge($bcc_emails, $provided_bcc);
    }
    ...
    if (!empty($args['attachments']) && is_array($args['attachments'])) {
        ...
        foreach ($args['attachments'] as $attachment) {
            if (!empty($attachment['path']) && file_exists($attachment['path'])) {
                ...
                $content_type = !empty($attachment['type']) ? $attachment['type'] : mime_content_type($attachment['path']);

                $payload['attachments'][] = [
                    'filename' => $attach_name,
                    'content'  => base64_encode($file_content),
                ];
                error_log('Apex Resend Email: Attached file - ' . $attach_name);
            }
        }
    }

    error_log('Apex Resend Email: Attempting to send email to ' . implode(', ', $to_emails));
    ...
    if (is_wp_error($response)) {
        error_log('Apex Resend Email: WP Error - ' . $response->get_error_message());
        return false;
    }
    ...
    if ($status_code === 200 || $status_code === 201) {
        error_log('Apex Resend Email: Email sent successfully. ID: ' . (isset($body['id']) ? $body['id'] : 'N/A'));
        return true;
    } else {
        $error_message = isset($body['message']) ? $body['message'] : 'Unknown error (HTTP ' . $status_code . ')';
        error_log('Apex Resend Email: Email sending failed - ' . $error_message);
        return false;
    }
}
...
/**
 * Test email function - call this manually to test Resend API setup
 */
function apex_test_email() {
    if (isset($_GET['test_email']) && current_user_can('administrator')) {
        $to = 'info@apex-softwares.com';
        $subject = 'Test Email from Apex Website';
        $message = '<h1>Test Email</h1><p>This is a test email to verify Resend API configuration.</p>';
        ...
        if ($sent) {
            echo 'Email sent successfully via Resend API!';
        } else {
            echo 'Email sending failed! Check error logs for details.';
        }
        exit;
    }
}
